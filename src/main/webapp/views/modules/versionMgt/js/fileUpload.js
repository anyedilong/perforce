var server_upload = $.util.basePath + '/r/vmm/upload';
jQuery(function() {
    function e(e) {
        var a = o('<li id="' + e.id + '"><p class="title">' + e.name + '</p><p class="imgWrap"></p><p class="progress"><span></span></p></li>'),
        s = o('<div class="file-panel"><span class="cancel">删除</span></div>').appendTo(a),
        ...
        function(t, n) {
            ...
            "error" === t || "invalid" === t ? 
            		(console.log(e.statusText), d(e.statusText), w[e.id][1] = 1) : "interrupt" === t ?
            				d("interrupt") : "queued" === t ? w[e.id][1] = 0 : "progress" === t ? 
            						(r.remove(), i.css("display", "block")) : "complete" === t && a.append('<span class="success"></span>'),
            a.removeClass("state-" + n).addClass("state-" + t), s.stop().animate({height: 30})
        }),
        a.on("mouseenter",
        function() {
            s.stop().animate({
    ...
    function i() {
        var e, a = "";
        "ready" === k ? a = "选中" + m + "个文件，共" + WebUploader.formatSize(h) + "。": "confirm" === k ? (e = n.getStats(), e.uploadFailNum && (a = "已成功上传" + e.successNum + "个文件，" + e.uploadFailNum + '个文件上传失败，<a class="retry" href="#">重新上传</a>失败图片或<a class="ignore" href="#">忽略</a>')) : (e = n.getStats(), a = "共" + m + "个文件（" + WebUploader.formatSize(h) + "），已上传" + e.successNum + "个文件", e.uploadFailNum && (a += "，失败" + e.uploadFailNum + "个文件")),
        p.html(a)
    }
    function t(e) {
        var a;
    ...
    n = WebUploader.create({
        ...
        fileSingleSizeLimit: 1024*1024*1024*4
    }),
    n.addButton({
        id: "#filePicker2",
        label: "继续添加"
    ...
    n.on( 'uploadSuccess', function( file,response) {//上传成功
    	
    	fileCount = fileCount-1,
        $( '#'+file.id ).addClass('upload-state-done');
       
        tt = $( '#'+file.id ).find("p.imgWrap");
        tt.text("点击下载！");
        var id = 
        
        $( '#'+file.id )[0].style.cursor = 'pointer'; 
        var versionId = $("#versionId").val();
        var status = response.status;
        if(status=='success'){//上传状态为成功
        	var fileinfo = response.fileInfo;//上传成功的文件
        	var fileId='';
        	for(var p in fileinfo){
        		
        		addfile(fileinfo[p].id);//保存数据
        		
        		fileId = fileId+fileinfo[p].id+',';
        		
        		tt. click(function(){ //下载
        			
        			var url =  fileinfo[p].serverUrl +fileinfo[p].filePath;
        			window.open(url);  
        		});
        		
        		var s1 = o('<div class="file-panel"><span class="cancel">删除</span></div>').appendTo($( '#'+file.id ));
        		var s1 = o('<div class="file-panel"><span class="cancel" onclick="delFile(\'\',\'' + fileinfo[p].id + '\',\'' + versionId + '\')">删除</span></div>').appendTo($( '#'+file.id )); //增加删除按钮
    ...
    n.onError = function(e) {
        if(e === "F_DUPLICATE"){
            ...
            alert("Eroor: " + "单次上传上限4GB！")
        }else if(e === "Q_EXCEED_SIZE_LIMIT"){
            alert("Eroor: " + "上传总上限20GB！")
        }else{
            alert("Eroor: " + e)
        }
