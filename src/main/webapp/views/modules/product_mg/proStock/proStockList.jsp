...
var gridTable;
	...
	function initProSlt(){
	 ...
	 //初始化机构树
   	...
   	var gridTableUrl = $.util.basePath + '/r/proStock/getFitLists';
    ...
    function initoption(){
		...
		var showUrl = $.util.basePath + '/r/proStock/showlist';
			var htmlDevice = [];
			htmlDevice.push('<option value="">全部</option>');
			$.util.ajax({
				url : showUrl,
				async: false,
				success : function(data) {
					if (!$.util.isNull(data) && data.length > 0) {
						for (var i = 0; i < data.length; i++) {
							var status = data[i];
							htmlDevice.push('<option value="'+status.value+'">'+ status.text + '</option>');
						}
					}
				}
			});

		$("#statusSlt").html(htmlDevice.join(''));
		$("#statusSlt").trigger("chosen:updated");
	}
	//刷新列表
